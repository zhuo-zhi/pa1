...
2) In this PA, print() returns a 32-bit integer, e.g. abs(print(-1)) while print() returns None in Python. To support this feature, we have to let the imports function return nothing.

3) In this PA, max()/min() doesn't support multiple arguments while Python can, e.g. max(1, 2, 3). To support this feature, we have to enable parsing multiple arguments in the ArgList for max()/min() and modify the processing logic in the corresponding import functions.
